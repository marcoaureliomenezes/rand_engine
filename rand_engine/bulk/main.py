...
if __name__ == '__main__':
  ...
  metadata = dict(
    ...
    job= dict(method=CoreDistincts.gen_distincts_typed, parms=dict(distinct=[fake.job() for _ in range(10)])),
    ...
    cpf= dict(method=CoreDistincts.gen_complex_distincts, 
      parms=dict(
        pattern="x.x.x-x",
        replacement="x",
        templates=[
  ...
  df = bulk_rand_engine.create_table(10**5, metadata)
  print(df)
