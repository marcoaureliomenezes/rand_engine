  ...
  def metadata_products(self, **kwargs):
    return {
      ...
      "category_id":      dict(
        ...
        kwargs=dict(distinct=DistinctUtils.handle_foreign_keys(table="categories", pk_fields=["category_id"]))),
      "client_id":      dict(
        method=Core.gen_distincts,
        kwargs=dict(distinct=DistinctUtils.handle_foreign_keys(table="clients", pk_fields=["client_id"])))
      }
