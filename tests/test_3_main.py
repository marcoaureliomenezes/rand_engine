...
def test_pandas_df_kwargs(df_size, rand_spec_case_1):
  """
  This test checks if the RandEngine can generate a pandas DataFrame with the specified number of rows
  using keyword arguments.
  """
  df_data = RandEngine(rand_spec_case_1).size(df_size).get_df()
...
def test_pandas_df_args(df_size, rand_spec_case_2):
  ...
  """
  df_data = RandEngine(rand_spec_case_2).size(df_size).get_df()
  print(df_data)
  assert df_data.shape[0] == df_size
...
def test_pandas_df_internal_transformer(df_size, rand_spec_case_1_transformer):
  ...
  """
  df_data = RandEngine(rand_spec_case_1_transformer).size(df_size).get_df()
  assert df_data.shape[0] == df_size
...
def test_pandas_df_constant(df_size, rand_spec_case_1):
  ...
  """
  df_data_1 = RandEngine(rand_spec_case_1, seed=True).size(df_size).get_df()
  df_data_2 = RandEngine(rand_spec_case_1, seed=True).size(df_size).get_df()
...
def test_pandas_df_constant_uuid(df_size, rand_spec_case_2):
  ...
  using a constant seed.
  """
  df_data_1 = RandEngine(rand_spec_case_2, seed=True).size(df_size).get_df()
  df_data_2 = RandEngine(rand_spec_case_2, seed=True).size(df_size).get_df()
  assert df_data_1.equals(df_data_2)
  assert df_data_1.shape == df_data_2.shape
...
def test_pandas_df_variable(df_size, rand_spec_case_1):
  ...
  """
  df_data_1 = RandEngine(rand_spec_case_1).size(df_size).get_df()
  df_data_2 = RandEngine(rand_spec_case_1).size(df_size).get_df()
  assert df_data_1.shape == df_data_2.shape
...
def test_pandas_df_transformer(df_size, rand_spec_case_1, update_transformer):
  ...
  df_data_2 = RandEngine(rand_spec_case_1).transformers(transformers).size(df_size).get_df()
...
def test_splitable_benchmark_baseline(df_size, rand_engine_splitable_benchmark_baseline):
  ...
  df_data = RandEngine(rand_engine_splitable_benchmark_baseline).size(df_size).get_df()
...
def test_splitable_benchmark(df_size, rand_engine_splitable_benchmark):
  df_size = 10**6
  start_time = time.time()
  df_data = RandEngine(rand_engine_splitable_benchmark).size(df_size).get_df()
  print(f"Elapsed time: {time.time() - start_time} seconds")
  assert df_data.shape[0] == df_size


# def test_create_pandas_df_wsl(df_size, rand_spec_case_wsl, wsl_transformer):
#   df_data = RandEngine(rand_spec_case_wsl).generate_pandas_df(df_size).size(df_size).get_df()
